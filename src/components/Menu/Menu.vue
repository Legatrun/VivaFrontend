<template>
  ...
  <v-card color="headerform">
    <v-list class="menubarbuttons" dense nav>
      ...
      <v-list-group v-for="item in menuUsuarios" :key="item.title" :prepend-icon="item.icon" no-action>
        <template v-slot:activator>
          <v-list-item-content>
            <v-list-item-title style="font-size:1.2em; color:#c6ffc1" v-text="item.title"></v-list-item-title>
          </v-list-item-content>
        </template>
        <v-list-item v-for="subItem in menuListaUsuarios" :key="subItem.title" :to="subItem.path" no-action>
          <v-list-item-content>
            <v-list-item-title style="font-size:1.3em; color:white" v-text="subItem.title"></v-list-item-title>
          </v-list-item-content>
        </v-list-item>
