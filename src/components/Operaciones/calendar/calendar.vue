<template>
	<v-card>
		<v-toolbar color="primary" style="color:white">
			<v-toolbar-title class="font-large-color">Datos de calendar</v-toolbar-title>
			<v-divider></v-divider>
		...
		<v-data-table 	style="padding: 5px"
			...
			<template v-slot:top>
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn class="btn-small-color" color="buttonadd" v-on="on" @click="Insertar()">REGISTRAR calendar</v-btn>
					</template>
					<span>Adicionar nuevo registro de calendar</span>
				</v-tooltip>
			</template>
						...
						<v-layout wrap>
							<template v-if="operacion == 'Insert'">
								<v-flex sm12 style="padding: 5px">
									<v-text-field class="input-small" 
												...
												persistent-hint
												required
												@input="calendar.id = updateText(calendar.id)">
									</v-text-field>
								</v-flex>
							</template>
							<template v-else>
							...
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											...
											persistent-hint
											required
											@input="calendar.identification = updateText(calendar.identification)">
								</v-text-field>
							</v-flex>
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											...
											persistent-hint
											required
											@input="calendar.description = updateText(calendar.description)">
								</v-text-field>
							</v-flex>
								...
								<v-menu
									...
									<template v-slot:activator="{ on }">
										<v-text-field class="date-small" 
											v-model="calendar.createtimestamp"
									...
									<v-date-picker v-model="calendar.createtimestamp" no-title @input="menu_createtimestamp = false"></v-date-picker>
								</v-menu>
							</v-flex>
							<v-flex sm4 class="hidden-xs-only" style="padding: 5px">
								<v-menu
									...
									<template v-slot:activator="{ on }">
										<v-text-field class="date-small" 
											v-model="calendar.updatetimestamp"
									...
									<v-date-picker v-model="calendar.updatetimestamp" no-title @input="menu_updatetimestamp = false"></v-date-picker>
								</v-menu>
							</v-flex>
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											v-model="calendar.createuser"
											hint="Ingrese CREATEUSER"
											placeholder="CREATEUSER"
											clearable
											persistent-hint
											required
											@input="calendar.createuser = updateText(calendar.createuser)">
								</v-text-field>
							</v-flex>
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											v-model="calendar.updateuser"
											hint="Ingrese UPDATEUSER"
											placeholder="UPDATEUSER"
											clearable
											persistent-hint
											required
											@input="calendar.updateuser = updateText(calendar.updateuser)">
								</v-text-field>
							</v-flex>
						</v-layout>
					</v-card-text>
