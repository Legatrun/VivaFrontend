<template>
	<v-card>
		<v-toolbar color="primary" style="color:white">
			<v-toolbar-title class="font-large-color">Datos de calendarversion</v-toolbar-title>
			<v-divider></v-divider>
			<v-text-field class="input-small"
					v-model="buscarcalendarversion"
					append-icon="search"
		...
		<v-data-table 	style="padding: 5px"
			...
			<template slot="item" slot-scope="props">
				<tr>
					<!--<td class="datatable-items-small">{{ helper.showDataDescription(props.item.id,lstcalendarversion, id, descripcion)  }}</td>// Ejemplo de Uso de Helper Para obtener la Descripcion de una Tabla por medio de su Id-->
			...
			<template v-slot:top>
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn class="btn-small-color" color="buttonadd" v-on="on" @click="Insertar()">REGISTRAR calendarversion</v-btn>
					</template>
					<span>Adicionar nuevo registro de calendarversion</span>
				</v-tooltip>
			</template>
		...
		<v-dialog v-model="dialog" persistent max-width="50%">
			<v-card>
				<v-toolbar style="padding:10px" dark class="primary">
					<v-toolbar-title class="font-medium-color" >Formulario de calendarversion</v-toolbar-title>
				</v-toolbar>
				<v-divider></v-divider>
						...
						<v-layout wrap>
							<template v-if="operacion == 'Insert'">
								<v-flex sm12 style="padding: 5px">
									<v-text-field class="input-small" 
												...
												persistent-hint
												required
												@input="calendarversion.id = updateText(calendarversion.id)">
									</v-text-field>
								</v-flex>
							</template>
							<template v-else>
							...
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											...
											persistent-hint
											required
											@input="calendarversion.description = updateText(calendarversion.description)">
								</v-text-field>
							</v-flex>
							...
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											v-model="calendarversion.calendarid"
											hint="Ingrese CALENDARID"
											placeholder="CALENDARID"
											clearable
											persistent-hint
											required
											type="number"
											@input="calendarversion.calendarid = updateText(calendarversion.calendarid)">
								</v-text-field>
							</v-flex>
							<v-flex sm4 class="hidden-xs-only" style="padding: 5px">
								<v-menu
									...
									<template v-slot:activator="{ on }">
										<v-text-field class="date-small" 
											v-model="calendarversion.createtimestamp"
									...
									<v-date-picker v-model="calendarversion.createtimestamp" no-title @input="menu_createtimestamp = false"></v-date-picker>
								</v-menu>
							</v-flex>
							<v-flex sm4 class="hidden-xs-only" style="padding: 5px">
								<v-menu
									...
									<template v-slot:activator="{ on }">
										<v-text-field class="date-small" 
											v-model="calendarversion.updatetimestamp"
									...
									<v-date-picker v-model="calendarversion.updatetimestamp" no-title @input="menu_updatetimestamp = false"></v-date-picker>
								</v-menu>
							</v-flex>
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											v-model="calendarversion.createuser"
											hint="Ingrese CREATEUSER"
											placeholder="CREATEUSER"
											clearable
											persistent-hint
											required
											@input="calendarversion.createuser = updateText(calendarversion.createuser)">
								</v-text-field>
							</v-flex>
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											v-model="calendarversion.updateuser"
											hint="Ingrese UPDATEUSER"
											placeholder="UPDATEUSER"
											clearable
											persistent-hint
											required
											@input="calendarversion.updateuser = updateText(calendarversion.updateuser)">
								</v-text-field>
							</v-flex>
						</v-layout>
					</v-card-text>
...
<script src="./calendarversion.ts"></script>
<style scoped>
