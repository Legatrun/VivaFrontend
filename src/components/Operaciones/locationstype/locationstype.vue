<template>
	<v-card>
		<v-toolbar color="primary" style="color:white">
			<v-toolbar-title class="font-large-color">Datos de locationstype</v-toolbar-title>
			<v-divider></v-divider>
		...
		<v-data-table 	style="padding: 5px"
			...
			<template v-slot:top>
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn class="btn-small-color" color="buttonadd" v-on="on" @click="Insertar()">REGISTRAR locationstype</v-btn>
					</template>
					<span>Adicionar nuevo registro de locationstype</span>
				</v-tooltip>
			</template>
		...
		<v-dialog v-model="dialog" persistent max-width="50%">
			<v-card>
				<v-toolbar style="padding:10px" dark class="primary">
					<v-toolbar-title class="font-medium-color" >Formulario de locationstype</v-toolbar-title>
				</v-toolbar>
				<v-divider></v-divider>
							...
							<template v-if="operacion == 'Insert'">
								<v-flex sm12 style="padding: 5px">
									<v-text-field class="input-small" 
												...
												persistent-hint
												required
												@input="locationstype.id = updateText(locationstype.id)">
									</v-text-field>
								</v-flex>
							</template>
							<template v-else>
								...
								<v-menu
									...
									<template v-slot:activator="{ on }">
										<v-text-field class="date-small" 
											v-model="locationstype.createtimestamp"
									...
									<v-date-picker v-model="locationstype.createtimestamp" no-title @input="menu_createtimestamp = false"></v-date-picker>
								</v-menu>
							</v-flex>
							<v-flex sm4 class="hidden-xs-only" style="padding: 5px">
								<v-menu
									...
									<template v-slot:activator="{ on }">
										<v-text-field class="date-small" 
											v-model="locationstype.updatetimestamp"
									...
									<v-date-picker v-model="locationstype.updatetimestamp" no-title @input="menu_updatetimestamp = false"></v-date-picker>
								</v-menu>
							</v-flex>
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											v-model="locationstype.createuser"
											hint="Ingrese CREATEUSER"
											placeholder="CREATEUSER"
											clearable
											persistent-hint
											required
											@input="locationstype.createuser = updateText(locationstype.createuser)">
								</v-text-field>
							</v-flex>
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											v-model="locationstype.updateuser"
											hint="Ingrese UPDATEUSER"
											placeholder="UPDATEUSER"
											clearable
											persistent-hint
											required
											@input="locationstype.updateuser = updateText(locationstype.updateuser)">
								</v-text-field>
							</v-flex>
						</v-layout>
					</v-card-text>
