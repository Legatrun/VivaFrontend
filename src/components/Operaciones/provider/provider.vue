<template>
	<v-card>
		<v-toolbar color="primary" style="color:white">
			<v-toolbar-title class="font-large-color">Datos de provider</v-toolbar-title>
			<v-divider></v-divider>
			<v-text-field class="input-small"
					v-model="buscarprovider"
					append-icon="search"
		...
		<v-data-table 	style="padding: 5px"
			...
			<template slot="item" slot-scope="props">
				<tr>
					<!--<td class="datatable-items-small">{{ helper.showDataDescription(props.item.identification,lstprovider, id, descripcion)  }}</td>// Ejemplo de Uso de Helper Para obtener la Descripcion de una Tabla por medio de su Id-->
			...
			<template v-slot:top>
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn class="btn-small-color" color="buttonadd" v-on="on" @click="Insertar()">REGISTRAR provider</v-btn>
					</template>
					<span>Adicionar nuevo registro de provider</span>
				</v-tooltip>
			</template>
		...
		<v-dialog v-model="dialog" persistent max-width="50%">
			<v-card>
				<v-toolbar style="padding:10px" dark class="primary">
					<v-toolbar-title class="font-medium-color" >Formulario de provider</v-toolbar-title>
				</v-toolbar>
				<v-divider></v-divider>
						...
						<v-layout wrap>
							<template v-if="operacion == 'Insert'">
								<v-flex sm12 style="padding: 5px">
									<v-text-field class="input-small" 
												...
												persistent-hint
												required
												@input="provider.identification = updateText(provider.identification)">
									</v-text-field>
								</v-flex>
							</template>
							<template v-else>
							...
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											...
											persistent-hint
											required
											@input="provider.description = updateText(provider.description)">
								</v-text-field>
							</v-flex>
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											...
											persistent-hint
											required
											@input="provider.type = updateText(provider.type)">
								</v-text-field>
							</v-flex>
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											...
											required
											type="number"
											@input="provider.enabled = updateText(provider.enabled)">
								</v-text-field>
							</v-flex>
								...
								<v-menu
									...
									<template v-slot:activator="{ on }">
										<v-text-field class="date-small" 
											v-model="provider.createtimestamp"
									...
									<v-date-picker v-model="provider.createtimestamp" no-title @input="menu_createtimestamp = false"></v-date-picker>
								</v-menu>
							</v-flex>
							<v-flex sm4 class="hidden-xs-only" style="padding: 5px">
								<v-menu
									...
									<template v-slot:activator="{ on }">
										<v-text-field class="date-small" 
											v-model="provider.updatetimestamp"
									...
									<v-date-picker v-model="provider.updatetimestamp" no-title @input="menu_updatetimestamp = false"></v-date-picker>
								</v-menu>
							</v-flex>
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											v-model="provider.createuser"
											hint="Ingrese CREATEUSER"
											placeholder="CREATEUSER"
											clearable
											persistent-hint
											required
											@input="provider.createuser = updateText(provider.createuser)">
								</v-text-field>
							</v-flex>
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											v-model="provider.updateuser"
											hint="Ingrese UPDATEUSER"
											placeholder="UPDATEUSER"
											clearable
											persistent-hint
											required
											@input="provider.updateuser = updateText(provider.updateuser)">
								</v-text-field>
							</v-flex>
						</v-layout>
					</v-card-text>
