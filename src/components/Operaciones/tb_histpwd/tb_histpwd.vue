<template>
	<v-card>
		<v-toolbar color="primary" style="color:white">
			<v-toolbar-title class="font-large-color">Datos de tb_histpwd</v-toolbar-title>
			<v-divider></v-divider>
			<v-text-field class="input-small"
					v-model="buscartb_histpwd"
					append-icon="search"
		...
		<v-data-table 	style="padding: 5px"
			...
			<template slot="item" slot-scope="props">
				<tr>
					<!--<td class="datatable-items-small">{{ helper.showDataDescription(props.item.nombre_usuario,lsttb_histpwd, id, descripcion)  }}</td>// Ejemplo de Uso de Helper Para obtener la Descripcion de una Tabla por medio de su Id-->
			...
			<template v-slot:top>
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn class="btn-small-color" color="buttonadd" v-on="on" @click="Insertar()">REGISTRAR tb_histpwd</v-btn>
					</template>
					<span>Adicionar nuevo registro de tb_histpwd</span>
				</v-tooltip>
			</template>
		...
		<v-dialog v-model="dialog" persistent max-width="50%">
			<v-card>
				<v-toolbar style="padding:10px" dark class="primary">
					<v-toolbar-title class="font-medium-color" >Formulario de tb_histpwd</v-toolbar-title>
				</v-toolbar>
				<v-divider></v-divider>
						...
						<v-layout wrap>
							<template v-if="operacion == 'Insert'">
								<v-flex sm12 style="padding: 5px">
									<v-text-field class="input-small" 
												v-model="tb_histpwd.nombre_usuario"
												...
												persistent-hint
												required
												@input="tb_histpwd.nombre_usuario = updateText(tb_histpwd.nombre_usuario)">
									</v-text-field>
								</v-flex>
							</template>
							<template v-else>
							...
							<v-flex sm4 class="hidden-xs-only" style="padding: 5px">
								<v-menu
									...
									<v-date-picker v-model="tb_histpwd.creacion" no-title @input="menu_creacion = false"></v-date-picker>
								</v-menu>
							</v-flex>
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											v-model="tb_histpwd.hash"
											hint="Ingrese HASH"
											placeholder="HASH"
											clearable
											persistent-hint
											required
											@input="tb_histpwd.hash = updateText(tb_histpwd.hash)">
								</v-text-field>
							</v-flex>
							<v-flex sm4 class="hidden-xs-only" style="padding: 5px">
								<v-menu
									...
									<template v-slot:activator="{ on }">
										<v-text-field class="date-small" 
											v-model="tb_histpwd.fec_modificacion"
									...
									<v-date-picker v-model="tb_histpwd.fec_modificacion" no-title @input="menu_fec_modificacion = false"></v-date-picker>
								</v-menu>
							</v-flex>
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											v-model="tb_histpwd.usua_modificacion"
											hint="Ingrese USUA_MODIFICACION"
											placeholder="USUA_MODIFICACION"
											clearable
											persistent-hint
											required
											@input="tb_histpwd.usua_modificacion = updateText(tb_histpwd.usua_modificacion)">
								</v-text-field>
							</v-flex>
						</v-layout>
					</v-card-text>
...
<script src="./tb_histpwd.ts"></script>
<style scoped>
