<template>
	<v-card>
		<v-toolbar color="primary" style="color:white">
			<v-toolbar-title class="font-large-color">Datos de tb_perfiles</v-toolbar-title>
			<v-divider></v-divider>
			<v-text-field class="input-small"
					v-model="buscartb_perfiles"
					append-icon="search"
		...
		<v-data-table 	style="padding: 5px"
						:headers="headers" 
						:items="lsttb_perfiles" 
						:items-per-page="30"
						:search = "buscartb_perfiles" 
						:footer-props="{
			...
			<template slot="item" slot-scope="props">
				<tr>
					<!--<td class="datatable-items-small">{{ helper.showDataDescription(props.item.id,lsttb_perfiles, id, descripcion)  }}</td>// Ejemplo de Uso de Helper Para obtener la Descripcion de una Tabla por medio de su Id-->
			...
			<template v-slot:top>
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn class="btn-small-color" color="buttonadd" v-on="on" @click="Insertar()">REGISTRAR tb_perfiles</v-btn>
					</template>
					<span>Adicionar nuevo registro de tb_perfiles</span>
				</v-tooltip>
			</template>
		...
		<v-dialog v-model="dialog" persistent max-width="50%">
			<v-card>
				<v-toolbar style="padding:10px" dark class="primary">
					<v-toolbar-title class="font-medium-color" >Formulario de tb_perfiles</v-toolbar-title>
				</v-toolbar>
				<v-divider></v-divider>
				<v-form ref="form" style="padding:10px">
					<v-card-text>
						<v-layout wrap>
							<template v-if="operacion == 'Insert'">
								<v-flex sm12 style="padding: 5px">
									<v-text-field class="input-small" 
												...
												persistent-hint
												required
												@input="tb_perfiles.id = updateText(tb_perfiles.id)">
									</v-text-field>
								</v-flex>
							</template>
							<template v-else>
							...
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											v-model="tb_perfiles.descripcion"
											hint="Ingrese DESCRIPCION"
											placeholder="DESCRIPCION"
											clearable
											persistent-hint
											required
											@input="tb_perfiles.descripcion = updateText(tb_perfiles.descripcion)">
								</v-text-field>
							</v-flex>
								...
								<v-menu
									...
									<template v-slot:activator="{ on }">
										<v-text-field class="date-small" 
											v-model="tb_perfiles.create_timestamp"
									...
									<v-date-picker v-model="tb_perfiles.create_timestamp" no-title @input="menu_create_timestamp = false"></v-date-picker>
								</v-menu>
							</v-flex>
							<v-flex sm4 class="hidden-xs-only" style="padding: 5px">
								<v-menu
									...
									<template v-slot:activator="{ on }">
										<v-text-field class="date-small" 
											v-model="tb_perfiles.fec_modificacion"
									...
									<v-date-picker v-model="tb_perfiles.fec_modificacion" no-title @input="menu_fec_modificacion = false"></v-date-picker>
								</v-menu>
							</v-flex>
							<v-flex sm12 style="padding: 5px">
								<v-text-field class="input-small" 
											v-model="tb_perfiles.usua_modificacion"
											hint="Ingrese USUA_MODIFICACION"
											placeholder="USUA_MODIFICACION"
											clearable
											persistent-hint
											required
											@input="tb_perfiles.usua_modificacion = updateText(tb_perfiles.usua_modificacion)">
								</v-text-field>
							</v-flex>
						</v-layout>
					</v-card-text>
...
<script src="./tb_perfiles.ts"></script>
<style scoped>
