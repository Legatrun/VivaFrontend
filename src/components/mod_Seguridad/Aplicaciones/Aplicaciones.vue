  ...
  <v-card max-width="100%" style="margin:auto;">
    <v-toolbar color="contrast" dark>
      <v-toolbar-title>
        ...
        &nbsp; &nbsp;Datos de Aplicaciones</v-toolbar-title
      >
      <v-divider></v-divider>
      <v-text-field
        v-model="buscaraplicaciones"
        append-icon="search"
    ...
    >
      ...
      <template v-slot:top>
        <v-tooltip bottom>
          ...
          <span>Adicionar nuevo registro de aplicacion</span>
        </v-tooltip>
      </template>
    ...
    <v-dialog v-model="dialog" persistent max-width="50%">
      <v-card>
        <v-toolbar style="padding:10px" dark class="primary">
          <v-toolbar-title>Datos de Aplicaciones</v-toolbar-title>
        </v-toolbar>
        <v-divider></v-divider>
        <v-form
          ref="form"
            ...
            <v-layout wrap>
              <template v-if="operacion == 'Insert'">
                <!-- <v-flex sm12 style="padding: 5px">
									<v-text-field v-model="aplicaciones.idaplicacion"
												label="IdAplicacion"
												...
												persistent-hint
												required
												@input="aplicaciones.idaplicacion = updateText(aplicaciones.idaplicacion)">
									</v-text-field>
								</v-flex> -->
              </template>
              <template v-else>
              ...
              <v-flex sm12 style="padding: 5px">
                <v-text-field
                  ...
                  persistent-hint
                  required
                  @input="aplicaciones.nombre = updateText(aplicaciones.nombre)"
              ...
              <v-flex sm12 style="padding: 5px">
                <v-textarea
                  v-model="aplicaciones.descripcion"
                  label="Descripcion"
                  ...
                  counter
                  @input="
                    aplicaciones.descripcion = updateText(
                      aplicaciones.descripcion
                    )
                  "
                  :rules="validacionDescripcionArray"
                >
                </v-textarea>
              </v-flex>
            </v-layout>
...
<script src="./Aplicaciones.ts"></script>
